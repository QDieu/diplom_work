...
#define EPS 1e-6
	...
	void forcesCalc_openmp(std::vector<Point> & points, std::vector<Point> & resPoints, const int thread_number, int& dt) {
		...
		for (int i = 0; i < iterations; i++)
		{
			if (i % 2 == 0) {
				...
				#pragma omp parallel num_threads(THREADS_NUMBER)
				{
					...
					forcesCalc_openmp(dataFirst, dataSecond, rank, dt);
				}
			}
			else {
				...
				{
					int rank = omp_get_thread_num();
					forcesCalc_openmp(dataSecond, dataFirst, rank, dt);
				}
			}
